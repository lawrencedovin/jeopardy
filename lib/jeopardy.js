...
async function getCategory(catId) {
  const response = await axios.get(`${BASE_URL}category`, { params: { id: `${catId}` }});
  const { data } = response;
  const { clues } = data;
  let categories = clues.map((result) => {
    ...
    return {
      title: title,
      clues: [
        { question: clues.question, answer: clues.answer, showing: null },
      ],
    };
  });
  return categories;
...
async function fillTable() {
  ...
  for(let i = 0; i < NUM_CATEGORIES; i++) {
      $theadrow.append(`<th>${categories[i][4].title}</th>`);
  }
