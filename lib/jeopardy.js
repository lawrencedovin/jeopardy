...
async function fillTable() {
  ...
  for (let i = 0; i < NUM_CLUES; i++) {
    ...
    for (let j = 0; j < NUM_CATEGORIES; j++) {
      let $td = $("<td>?</td>").addClass("question-mark").attr("id", `${j}-${i}`);
      $tbodyrow.append($td);
    }
  }
...
function handleClick(evt) {
  ...
  if (clueData.showing === null) {
    ...
    $(this).text(clueData.question).removeClass("question-mark").addClass("question");
  } else if (clueData.showing === "question") {
    clueData.showing = "answer";
    $(this).text(clueData.answer).removeClass("question-mark").addClass("answer");
  } else return;
}
...
function createResetButton(){
  const $buttonContainer = $('<div>').addClass("button-container d-flex justify-content-center");
  const $button = $('<button>RESET</button>').addClass("btn btn-primary btn-lg");
