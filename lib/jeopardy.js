...
async function fillTable() {
  ...
  for (let i = 0; i < NUM_CLUES; i++) {
    ...
    for (let j = 0; j < NUM_CATEGORIES; j++) {
      let $td = $("<td>?</td>").addClass("question").attr("id", `${j}-${i}`);
      $tbodyrow.append($td);
    }
  }
...
function handleClick(evt) {
  ...
  if (clueData.showing === null) {
    ...
    $(this).text(clueData.question).removeClass("question").addClass("clue");
  } else if (clueData.showing === "question") {
    clueData.showing = "answer";
    $(this).text(clueData.answer).removeClass("question").addClass("clue");
  } else return;
}
